...
export async function getPosts(
  ...
  const res = await fetch(
    `${process.env.BASE_API_URL}/blog/posts?page=${page}&status=${status}`,
    {
      cache: "no-cache",
      headers: {
...
export async function getPublishedPosts(page: string = "1"): Promise<TPost[]> {
  const cookie = cookies().get("jwtToken")?.value || "";
  const res = await fetch(
    `${process.env.BASE_API_URL}/blog/posts?page=${page}`,
    {
      cache: "no-cache",
      headers: {
        Authorization: `Bearer ${cookie}`,
...
export async function getHiddenPosts(page: string = "1"): Promise<TPost[]> {
  const cookie = cookies().get("jwtToken")?.value || "";
  const res = await fetch(
    `${process.env.BASE_API_URL}/blog/posts?page=${page}&status=HIDDEN`,
    {
      cache: "no-cache",
      headers: {
        Authorization: `Bearer ${cookie}`,
...
export async function getDraftPosts(page: string = "1"): Promise<TPost[]> {
  const cookie = cookies().get("jwtToken")?.value || "";
  const res = await fetch(
    `${process.env.BASE_API_URL}/blog/posts?page=${page}&status=DRAFT`,
    {
      cache: "no-cache",
      headers: {
        Authorization: `Bearer ${cookie}`,
...
export async function getPostsCount(
  ...
  const res = await fetch(
    `${process.env.BASE_API_URL}/blog/posts/count?status=${status}`,
    {
      cache: "no-cache",
    }
  );
...
export async function getPostById(postId: string | number): Promise<TPost> {
  ...
  const cookie = cookies().get("jwtToken")?.value || "";
  const res = await fetch(`${process.env.BASE_API_URL}/blog/posts/${postId}`, {
    cache: "no-cache",
    headers: {
      Authorization: `Bearer ${cookie}`,
    },
...
export async function getPostCommentsById(
  ...
  const cookie = cookies().get("jwtToken")?.value || "";
  const res = await fetch(
    `${process.env.BASE_API_URL}/blog/posts/${postId}/comments`,
    {
      cache: "no-cache",
      headers: {
        Authorization: `Bearer ${cookie}`,
...
export async function getPostsByTitle(title: string): Promise<TPost[]> {
  ...
  const res = await fetch(
    `${process.env.BASE_API_URL}/blog/posts/search?query=${title}`,
    {
      cache: "no-cache",
      headers: {
        Authorization: `Bearer ${cookie}`,
...
export async function getComments(page: string = "1"): Promise<TComment[]> {
  const cookie = cookies().get("jwtToken")?.value || "";
  const res = await fetch(
    `${process.env.BASE_API_URL}/blog/comments?page=${page}`,
    {
      cache: "no-cache",
      headers: {
        Authorization: `Bearer ${cookie}`,
...
export async function getCommentsCount(): Promise<{ count: number }> {
  const res = await fetch(`${process.env.BASE_API_URL}/blog/comments/count`, {
    cache: "no-cache",
  });
